...
<asset id="r2" name="DJI_20260405215615_0013_D"
  ...
  <media-rep kind="original-media" sig="04972A01EBD8BD56BFD94A8B58AC451F"
             src="file:///Volumes/KIOXIA/srilanka/Video/DJI_20260405215615_0013_D.MP4"/>
</asset>
...
## 5. プロジェクト固有情報（スリランカVlog）

現在進行中の travelVlog（`~/Desktop/work/work/KPLS/artifacts/スリランカVlog設計書.md`）で使う前提の配置：

```
/Volumes/KIOXIA/srilanka/
├── srilanka.fcpbundle      FCPライブラリ
├── MusicSE/                 音声素材（BGM, 環境音, SE）
├── Graphics/                静止画グラフィック（hook-title, card-day*, route-map等）
├── Video/                   DJI素材コピー先（SD依存を避けるため）
├── full-skeleton.fcpxml    粗編集スケルトン
└── *.fcpxml.bak            バックアップ
```

- 設計書ドラフトからの FCPXML 化は travelVlog skill の管轄
- FCPXML の**書き方と修正**はこのスキルの管轄
...
より詳しい背景・過去の失敗譚・なぜこう書くのかは、セッション横断メモリに格納してある：

- `feedback_fcpxml_rules.md` — FCPXML 1.13 の基本制約（静止画の扱い、gap構造、DTD）
- `feedback_fcpxml_framerate.md` — 29.97 素材の denominator 整合
- `feedback_fcpxml_asset_start_tc.md` — asset.start = camera TC（本スキル最大の教訓）
- `feedback_fcpxml_probe_first.md` — Probe XML で仕様を確定させる流儀
- `feedback_fcpxmld_bundle.md` — `.fcpxmld` バンドル形式を作業基準にする運用
- `feedback_fcpxml_resource_id_dynamic.md` — resource ID は動的取得すべき（固定書き禁止）
- `feedback_design_doc_fidelity.md` — 設計書の核を落とさずドラフト化する原則
